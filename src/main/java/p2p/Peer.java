...
public class Peer {
  ...
  public List<Block> tempStorage = new ArrayList<>();
  ...
  public String listTempBlocks() {
    String str = "Blocks->\n";
    for (Block entry : this.tempStorage) {
      str += (entry + "\n");
    }
    return str;
  }
  ...
  private void postOffice() throws IOException {
    while (true) {
      try {
        ...
        InetSocketAddress receiver =
          (Util.deserializeNode(message.getLocalNode())).getAddr();
        ...
        switch (message.getType()) {
          ...
          case PING_REPLY:
            callers.put(
              Util.deserializeNode(message.getLocalNode()).getId(),
              Util.deserializeNode(message.getLocalNode())
            );
            message.setType(STOP);
            if (socketChannel != null) socketChannel.write(
              ByteBuffer.wrap(
                ("Receiving Ping from \n" + localNode.toString()).getBytes(
          ...
          case SHOW:
            if (socketChannel != null) socketChannel.write(
              ByteBuffer.wrap(message.getContent())
            );
            break;
          default:
            if (socketChannel != null) socketChannel.write(
              ByteBuffer.wrap(
                ("Unexpected value:\n" + message.getType()).getBytes(
                    StandardCharsets.UTF_8
                  )
              )
            );
